...
> Duplicate-Finder - Open source tool by AetherCodeHQ

`cli` `deduplication` `files` `golang` `utilities`

## What is it?

**Duplicate Finder** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Duplicate-Finder.git
cd Duplicate-Finder
go build -o Duplicate-Finder .
```
...
```bash
# Basic usage
./Duplicate-Finder --path ./target

# With options
./Duplicate-Finder --path ./target --format json --output report.json

# Verbose mode
./Duplicate-Finder --path ./target --verbose
```
...
### Basic scan
```bash
./Duplicate-Finder --path ./my-project
```

### JSON report
```bash
./Duplicate-Finder --path ./my-project --format json --output report.json
```
...
- name: Run Duplicate Finder
  run: ./Duplicate-Finder --path . --format json --output report.json
```
